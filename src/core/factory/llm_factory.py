...
class InitLLM:
    ...
    def configure() -> Any:
        ...
        custom_client = AsyncOpenAI(api_key=settings.LLM_API_KEY, base_url=settings.LLM_BASE_URL)

        # Wrapping the client into the framework's specific Model class
        model = OpenAIChatCompletionsModel(model=settings.LLM_MODEL_NAME, openai_client=custom_client)
